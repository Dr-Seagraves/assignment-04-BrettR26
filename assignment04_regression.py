...
def plot_scatter_with_regression(
    ...
    ax.plot(x_range, y_range, 'r-', linewidth=2, label=f'Fitted Line')
    
    # Set axis limits to central data (2nd–98th percentiles)
    x_2 = plot_df[x_var].quantile(0.02)
    x_98 = plot_df[x_var].quantile(0.98)
    y_2 = plot_df['ret'].quantile(0.02)
    y_98 = plot_df['ret'].quantile(0.98)
    ax.set_xlim(x_2, x_98)
    ax.set_ylim(y_2, y_98)
    
    # Add title (include R²), labels, and legend
...
def print_key_results(model, x_var: str) -> None:
    ...
    print(f"Intercept (β₀):  {intercept:10.6f}  (SE: {intercept_stderr:.6f})")
    print(f"               t-stat: {intercept_tstat:8.4f}  p-value: {intercept_pval:.6f}")
    print()
    print(f"Slope (β₁):      {slope:10.6f}  (SE: {slope_stderr:.6f})")
    print(f"               t-stat: {slope_tstat:8.4f}  p-value: {slope_pval:.6f}")
    print()
    ...
    sig_5pct = slope_pval < 0.05
    sign_str = "positive" if slope > 0 else "negative"
    sig_str = "significant" if sig_5pct else "not significant"
    print(f"Slope is {sign_str} and {sig_str} at the 5% level.")
    print("=" * 60 + "\n")
